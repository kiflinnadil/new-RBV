...
<div class="bg-gray-50 min-h-screen py-8 sm:py-12 lg:py-16">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-[20px] sm:rounded-[30px] shadow-xl border border-gray-100
            ...
            <form action="{{ route('promkes.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
...
<script>
document.getElementById('filePromkesCreate').addEventListener('change', function (e) {
    const file = e.target.files[0];
    if (file) {
        document.getElementById('fileTextPromkesCreate').innerText = file.name;
    }
});
</script>
