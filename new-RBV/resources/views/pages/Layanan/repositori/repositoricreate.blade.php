                ...
                <div class="space-y-4 sm:space-y-6">
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            placeholder="Masukkan deskripsi karya">{{ old('deskripsi') }}</textarea>
                        @error('deskripsi')
                            <p class="text-red-500 text-xs mt-1 ml-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <label class="flex items-center gap-3 w-full bg-gray-100 rounded-xl
                            ...
                            <span id="fileTextRepoCreate" class="text-gray-400 text-xs sm:text-sm italic truncate">
                                File Maksimal 20 MB
                            </span>
                            <input type="file" name="file" id="fileRepoCreate" class="hidden" accept=".pdf">
                        </label>
                        @error('file')
                            <p class="text-red-500 text-xs mt-1 ml-1">{{ $message }}</p>
                        @enderror
...
<script>
document.getElementById('fileRepoCreate').addEventListener('change', function (e) {
    const file = e.target.files[0];
    if (file) {
        document.getElementById('fileTextRepoCreate').innerText = file.name;
    }
});
</script>
